...
export default function RegisterKomunitas() {
  ...
  const nextStep = () => {
    if (currentStep === 1) {
      if (!name || !email || !password) {
        alert("Mohon isi semua kolom pada langkah ini");
        return;
      }
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      ...
      if (!penanggungJawab || !telephone || !alamat || !tahun) {
        alert("Mohon isi semua kolom pada langkah ini");
        return;
      }
  ...
  const handleSubmit = async () => {
    // Validate all required fields for the final step
    if (!jmlAnggota || !desc || !logo) {
      alert("Mohon isi semua kolom pada langkah ini");
      return;
    }

    if (isNaN(parseInt(jmlAnggota))) {
      alert("Jumlah anggota harus berupa angka");
      return;
    }
    ...
    try {
      const yearEstablished = parseInt(tahun);
      const membersCount = parseInt(jmlAnggota);
      ...
      const response = await apiService.communities.register(communityData);
      ...
      if (error.response) {
        ...
        alert(`Error: ${error?.response.data.message || "Terjadi kesalahan saat mendaftar"}`);
      } else {
  ...
  const isStepValid = () => {
    if (currentStep === 1) {
      return name.trim() !== "" && email.trim() !== "" && password.length >= 6 && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
    }

    if (currentStep === 2) {
      return penanggungJawab.trim() !== "" && telephone.trim().length >= 10 && /^\d+$/.test(telephone) && alamat.trim() !== "" && tahun.trim() !== "" && /^\d+$/.test(tahun);
    }

    if (currentStep === 3) {
      return jmlAnggota.trim() !== "" && /^\d+$/.test(jmlAnggota) && desc.trim() !== "" && logo !== null && rememberMe;
    }
          ...
          <div className={styles.leftContent}>
            ...
            >
              <div className={styles.navigationButtons}>
                {currentStep < 3 ? (
                  <>
                    {currentStep > 1 && <ButtonMain btnText="Kembali" btnColor={true} textColor="white" onClick={prevStep} disabled={loading} />}{" "}
                    <ButtonMain btnText="Lanjut" btnColor={true} textColor="white" onClick={nextStep} disabled={!isStepValid() || loading} />
                  </>
                ) : (
                  <ButtonMain btnText="Daftar" btnColor={true} textColor="white" fullWidth onClick={handleSubmit} loading={loading} disabled={!isStepValid() || loading} />
                )}
              </div>
